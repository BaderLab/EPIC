...
class CORUM():
	...
	def getCORUM(self):
		...
		corum_url = urllib2.urlopen(corum_url)
		corum_url_FH = ZipFile(StringIO(corum_url.read()))
		for line in corum_url_FH.open("allComplexes.txt").readlines():
			line = line.rstrip()
...
class Clusters():
	...
	def sensitivity(self, reference):
		sum_ref_size = 0
		sum_all_max_overlapps = 0
		for reference_cluster in reference.complexes:
			max_overlapp = 0
			sum_ref_size += len(reference.complexes[reference_cluster])
			for predicted_cluster in self.complexes:
				this_overlap = len(self.complexes[predicted_cluster] & reference.complexes[reference_cluster])
				if this_overlap > max_overlapp: max_overlapp = this_overlap
			sum_all_max_overlapps += max_overlapp

		if sum_ref_size == 0:
			return 0
		else:
			return sum_all_max_overlapps/sum_ref_size
